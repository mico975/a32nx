class CDUVerticalRevisionPage {
    static ShowPage(mcdu, waypoint, verticalWaypoint, isDescentWaypoint) {
        ...
        if (waypointInfo instanceof WayPointInfo) {
            ...
            let speedLimitCell = "*[][color]cyan";

            const speedLimitToConsider = isDescentWaypoint ? mcdu.managedSpeedLimitDescent : mcdu.managedSpeedLimitClimb;
            const speedLimitAltToConsider = isDescentWaypoint ? mcdu.managedSpeedLimitAltDescent : mcdu.managedSpeedLimitAltClimb;

            if (isFinite(speedLimitToConsider) && isFinite(speedLimitAltToConsider)) {
                speedLimitCell = speedLimitToConsider + "/" + this.formatFl(speedLimitAltToConsider, transAltLevel) + "[color]magenta";
            }
            ...
            mcdu.setTemplate([
                ...
                [""],
                [""],
                ["\xa0" + (isDescentWaypoint ? "DES" : "CLB") + " SPD LIM", ""],
            ...
            mcdu.onLeftInput[1] = (value, scratchpadCallback) => {
                if (value === FMCMainDisplay.clrValue) {
                    if (isDescentWaypoint) {
                        mcdu.setDescentSpeedLimit(undefined, undefined);
                    } else {
                ...
                if (isDescentWaypoint) {
                    mcdu.setDescentSpeedLimit(speedLimit, speedLimitAlt);
                } else {
